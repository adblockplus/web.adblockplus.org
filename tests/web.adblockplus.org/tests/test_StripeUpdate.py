...
def test_monthly_donation_amount_under_minimum(driver):
    ...
    update_page.click_monthly_custom_button()
    update_page.enter_monthly_custom_value(td.MONTHLY_AMOUNT_UNDER_MINIMUM)

    assert update_page.get_monthly_amount_under_minimum_error_text ==\
           td.MONTHLY_AMOUNT_UNDER_MINIMUM_ERROR


def test_one_time_donation_amount_under_minimum(driver):
    update_page = UpdatePage(driver)
    update_page.click_one_time_custom_button()
    update_page.enter_one_time_custom_value(td.ONE_TIME_AMOUNT_UNDER_MINIMUM)

    assert update_page.get_one_time_amount_under_minimum_error_text ==\
...
def test_custom_monthly_donation_amount(driver, test_id, payment_value, submit_button_text):
    update_page = UpdatePage(driver)
    update_page.click_monthly_custom_button()
    update_page.enter_monthly_custom_value(payment_value)
    stripe_payments_form = update_page.click_stripe_button()
    ...
    assert update_page.is_redirect_to_thank_you()
...
def test_custom_one_time_donation_amount(driver, test_id, payment_value, submit_button_text):
    update_page = UpdatePage(driver)
    update_page.click_one_time_custom_button()
    update_page.enter_one_time_custom_value(payment_value)
    stripe_payments_form = update_page.click_stripe_button()
    stripe_payments_form.enter_email(td.TEST_EMAIL)
    stripe_payments_form.enter_card_number(td.TEST_CARD_NUMBER)
    stripe_payments_form.enter_card_exp(td.TEST_CARD_EXPIRY)
...
def test_fixed_one_time_donation_amounts(driver, payment_option, submit_button_text):
    update_page = UpdatePage(driver)
    update_page.click_radio_button(update_page.get_all_one_time_buttons[payment_option])
    stripe_payments_form = update_page.click_stripe_button()
    stripe_payments_form.enter_email(td.TEST_EMAIL)
    stripe_payments_form.enter_card_number(td.TEST_CARD_NUMBER)
    stripe_payments_form.enter_card_exp(td.TEST_CARD_EXPIRY)
...
def test_fixed_monthly_donation_amounts(driver, payment_option, submit_button_text):
    update_page = UpdatePage(driver)
    update_page.click_radio_button(update_page.get_all_monthly_buttons[payment_option])
    stripe_payments_form = update_page.click_stripe_button()
    stripe_payments_form.enter_email(td.TEST_EMAIL)
    stripe_payments_form.enter_card_number(td.TEST_CARD_NUMBER)
    stripe_payments_form.enter_card_exp(td.TEST_CARD_EXPIRY)
