...
function PaymentForm(currencies)
{
  ...
  function handleOneTimeCheckbox(e) {
    if (e.target.name == 'preset-monthly-subscription-amount') {
      doc.body.classList.add('monthly-selected');

    } else if (e.target.name == 'preset-yearly-subscription-amount') {
      doc.body.classList.remove('monthly-selected');
    }
  }
  ...
  subscriptionAmounts.addEventListener("change", handleOneTimeCheckbox);

  yearlySubscriptionAmounts.addEventListener("change", handleOneTimeCheckbox);
  ...
  {
    var oneTimeChecked = doc.getElementById('one-time').checked;
    ...
    if (oneTimeChecked && selectedType == 'yearly')
  ...
  doc.getElementById('one-time').addEventListener("change", function() {
    clearCustomAmountErrors();

    if (document.querySelector('#yearly-subscription-amounts input[value="custom"]').checked)
       doc.querySelector('input[name="custom-yearly-subscription-amount"]').focus();
  }, true);
  ...
  this.toJSON = function()
  {
    ...
    if (oneTimeChecked && (type == 'yearly-subscription'))
      type = 'donation';
