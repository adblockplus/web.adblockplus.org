...
const hasSubscriptionEndpoint = "https://is-premium-endpoint-dot-abp-payments.ey.r.appspot.com/user/has_active_premium_subcription";
...
function preventDuplicateSubscription(event) {
  ...
  setTimeout(() => {
    ...
    adblock.logServiceError("preventDuplicateSubscription", {reason: "timeout"});
  ...
  .then((result) => {
    if (response.ok) {
      ...
      adblock.logServiceError("preventDuplicateSubscription", {
        reason: "response",
  ...
  .catch((error) => {
    adblock.logScriptError("preventDuplicateSubscription", error);
    finishIntervention();
  });
