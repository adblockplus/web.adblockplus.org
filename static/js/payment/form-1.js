  ...
  */
  function PaymentForm(currencies)
  {
    ...
    paymentCurrency.addEventListener("change", function(e) {
      ...
      doc.querySelector(".stripe-button").click();
    });

    function paymentMethod(e) {
      if (e.target.value == "card") {
        doc.body.classList.add("paypal-cta-hidden");
        doc.body.classList.remove("card-cta-hidden");
      } else {
        doc.body.classList.add("card-cta-hidden");
        doc.body.classList.remove("paypal-cta-hidden");
      }
    }
    ...
    function enableForm(enabled)
    {
      ...
      _.each(
        _.toArray(doc.querySelectorAll(".pay-button")),
        function(button)
        {
          button.disabled = !enabled;
        }
    ...
    donationAmounts.addEventListener("keyup", function() {
      doc.querySelector(".stripe-button").click();
    }, true);

    subscriptionAmounts.addEventListener("keyup", function() {
      doc.querySelector(".stripe-button").click();
    }, true);

    yearlySubscriptionAmounts.addEventListener("keyup", function() {
      doc.querySelector(".stripe-button").click();
    }, true);
