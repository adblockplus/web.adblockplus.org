...
1. {{ data-processed-item-1[list item] While using our products: Adblock Plus or Adblock Browser (automatically, except for Issue Reporter) }}
    ...
    - {{ data-processed-item-9[list item] For all mobile products and Adblock Plus for Safari on macOS }}
        ...
        {: .has-horizontal-list .semicolon-separated }
            - {{ data-processed-item-12[list item] UUID and crash traces in order to help a user associate crash data with specific instances of our products }}
            ...
            - {{ data-processed-item-16[list item] App and/or extension version }}
        ...
        {: .has-horizontal-list .semicolon-separated }
            - {{ data-processed-item-21[list item] App and/or extension version }}
            - {{ data-processed-item-22[list item] Demographic data (gender, age) }}
            - {{ data-processed-item-23[list item] Device type }}
            - {{ data-processed-item-24[list item] Which platforms you are using }}
            - {{ data-processed-item-25[list item] Location data based on your IP address (country) }}
            - {{ data-processed-item-26[list item] Button clicks and/or navigation patterns }}
            - {{ data-processed-item-27[list item] Screens shown }}
    ...
    - {{ data-processed-item-28[list item] Adblock Browser for iOS (automatically): }}
    {: .has-horizontal-list .semicolon-separated }
        - {{ data-processed-item-29[list item] Error and crash report data: }}
            - {{ data-processed-item-30[list item] Bundle identifier, bundle version and short bundle version string of Adblock Browser }}
            - {{ data-processed-item-31[list item] Device type, CPU architecture and version of the operating system }}
            - {{ data-processed-item-32[list item] Timestamp of crash }}
            - {{ data-processed-item-33[list item] UUID }}
...
1. {{ data-processed-item-37[list item] While using our websites }}
    ...
    {: .has-horizontal-list .semicolon-separated }
        - {{ data-processed-item-39[list item] IP address (stored separately) }}
        - {{ data-processed-item-40[list item] The time at which the request was made }}
        - {{ data-processed-item-41[list item] The web address accessed }}
        - {{ data-processed-item-42[list item] The browser identifier }}
        - {{ data-processed-item-43[list item] The referring page }}
        - {{ data-processed-item-44[list item] Aggregated analytics data }}
    - {{ data-processed-item-45[list item] Voluntary: }}
    {: .has-horizontal-list .semicolon-separated }
        - {{ data-processed-item-46[list item] Forum registration data }}
        - {{ data-processed-item-47[list item] Email address }}
        - {{ data-processed-item-48[list item] Data you provide in comments }}
        - {{ data-processed-item-49[list item] Feedback and filter data }}
1. {{ data-processed-item-50[list item] For user support }}
    - {{ data-processed-item-51[list item] User name (contact via social media) and/or email }} 
    - {{ data-processed-item-52[list item] Device information }}
    - {{ data-processed-item-53[list item] Version of Adblock Plus and other applicable tech specifications }}
    - {{ data-processed-item-54[list item] URLs the user is having issues with }}
    - {{ data-processed-item-55[list item] Adblock Plus issues }}
    - {{ data-processed-item-56[list item] Address and transaction ID numbers for donation refunds (voluntary) }}
...
#### {{ mobile-products-heading[heading] All mobile products and for Adblock Plus for Safari on macOS }} {: #all-mobile-products }

##### {{ mobile-products-crash-reporter-heading[heading] Crash reporter }} {: #adblock-browser-crash-reporter }

{{ mobile-products-crash-reporter-paragraph-1 In the event of an unexpected crash or error, our mobile products and Adblock Plus for Safari on macOS collect data relevant for analyzing the cause of the malfunction. The data is used solely for individual troubleshooting (as well as for identifying general defects which lead to unexpected crashes or errors) and is never shared with any third parties other than Fabric from Google, Inc., which we use for analyzing crash reports (crashlytics). }}

###### {{ mobile-products-crash-data-heading[heading] During a crash, the following data is sent: }}

- {{ mobile-products-crash-data-item-1[list item] UUID and crash traces in order to help a user associate crash data with specific instances of our mobile products and Adblock Plus for Safari on macOS }}
- {{ mobile-products-crash-data-item-2[list item] Instance IDS }}
- {{ mobile-products-crash-data-item-3[list item] Device type }}
- {{ mobile-products-crash-data-item-4[list item] Application ID }}
- {{ mobile-products-crash-data-item-5[list item] App and/or extension version }}
- {{ mobile-products-crash-data-item-6[list item] Free space }}
- {{ mobile-products-crash-data-item-7[list item] Free ram }}
- {{ mobile-products-crash-data-item-8[list item] A timestamp of when a crash happened }}

{{ mobile-products-crash-data-paragraph-1 Said data is only collected when the application crashes or throws an error. You can opt-out from sending such reports via Settings > Privacy. }}

###### {{ mobile-products-crash-data-retention-heading[heading] Data retention }}

{{ mobile-products-crash-data-retention-paragraph-1 Crash reporting is retained for 180 days. Firebase retains Instance IDs until the Firebase customer makes an API call to delete the ID. After the call, data is removed from live and backup systems within 180 days. }}

##### {{ mobile-products-event-tracking-heading[heading] Event tracking }}

{{ mobile-products-event-tracking-paragraph-1 We also track when you create an event, e.g. when you access the settings. In such a case, this event will be sent to our service provider, Firebase from Google, Inc. Firebase then makes the following data accessible to us: }}

- {{ mobile-products-event-tracking-item-1[list item] App and/or extension version }}
- {{ mobile-products-event-tracking-item-2[list item] Demographic data (gender, age) }}
- {{ mobile-products-event-tracking-item-3[list item] Device type }}
- {{ mobile-products-event-tracking-item-4[list item] Which platforms you are using (Android and its respective version) }}
- {{ mobile-products-event-tracking-item-5[list item] Location data based on your IP address (country) }}
- {{ mobile-products-event-tracking-item-6[list item] Button clicks and/or navigation patterns }}
- {{ mobile-products-event-tracking-item-7[list item] Screens shown }}

{{ mobile-products-event-tracking-paragraph-2 This data is solely used to improve our mobile products and Adblock Plus for Safari on macOS and your user experience. You can opt-out from sending such reports via Settings > Privacy > Help us make Adblock Browser better. }}
...
###### {{ retention-crash-reports-heading[heading] Data retention: }} {: #retention-crash-reports }
...
<time datetime="2019-06">{{ publication-date October 2019 }}</time>
