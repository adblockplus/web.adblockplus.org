...
1. {{ data-processed-item-1[list item] While using our products (extension or Adblock Browser) }}
    - {{ data-processed-item-2[list item] Extension updates, subscription downloads, emergency notifications: }}
      <div class="inline-list" markdown="1">
        ...
        - {{ data-processed-item-8[list item] For Issue reporter submitted manually by you, additionally: address of the webpage with the bug, blockable items on the webpage, matching filters, active subscription }}
      </div>
    ...
    - {{ data-processed-item-11[list item] Adblock Browser for iOS: }}
...
### {{ why-we-collect-heading[heading] Why do we need to collect your data? }} {: #why-we-collect-data }

- {{ why-we-collect-item-1[list item] To provide our services to you }}
- {{ why-we-collect-item-2[list item] To improve and to evaluate our products }}
- {{ why-we-collect-item-3[list item] To improve our website and to ensure website security }}
...
- [{{ general-notes-item-2[list item & link] Who is responsible for data processing? }}](#who-is-responsible)
- [{{ general-notes-item-3[list item & link] What is personal data? }}](#what-is-personal-data)
- [{{ general-notes-item-4[list item & link] What rights do you have? }}](#your-rights-long)
- [{{ general-notes-item-5[list item & link] Deletion of your data }}](#deletion-of-data)
- [{{ general-notes-item-6[list item & link] Changes to this Privacy Policy }}](#changes-to-policy)
- [{{ general-notes-item-7[list item & link] Collection and processing in our products }}](#collection-and-processing)
...
### {{ who-is-responsible-heading[heading] Who is responsible for data processing? }} {: #who-is-responsible }

{{ who-is-responsible-paragraph-1 The legal person responsible for the collection, processing and / or use of personal data in connection with <fix>Adblock Plus</fix> ("Controller") is: }}
...
{{ issue-reporter-paragraph-1 Adblock Plus for Firefox allows you to send issue reports that will be temporarily stored on the AdblockPlus.org domain. These reports contain information required to investigate the issue including the Adblock Plus version, browser version, address of the web page where the problem is visible, blockable items on the web page, matching filters and active subscriptions. Parameter values are removed from all transmitted addresses to avoid unintentionally leaking private information. It is also possible to provide additional information on a voluntary basis, which may be privacy-sensitive: }}
...
{{ error-data-sent-paragraph-1 Said data is only collected when the application crashes or throws an error. Furthermore, Adblock Browser for iOS explicitly asks for permission to send the collected information after the crash or error happened. All reports stored on our servers are removed after 30 days. }}
...
###### {{ stored-website-requests-heading[heading] Stored Website request }} {: #stored-website-requests }
...
{{ forum-registration-paragraph-3 **Note:** Registration is not required to participate in the forums. }}
...
<time>{{ publication-date May 2017 }}</time>
